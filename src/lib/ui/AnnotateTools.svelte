<script lang="ts">
	...
	import { type Annotation, KINDS, KIND_IDS, newAnnotation } from '$lib/annotate/annotation';
	import { type DrawHandle, type DrawModeName, createDraw, roundPosition, toSourceData } from '$lib/annotate/draw';
	import { type AnnotateKey, at } from '$lib/annotate/messages';
	import { AnnotationStore, KIND_KEYS } from '$lib/annotate/store.svelte';
	import { browserStore } from '$lib/annotate/storage';
...
<style>
	...
	/*
	 * Both of MapLibre's top corners are taken: the settings controls sit top-left
	 * and the zoom buttons top-right. On a phone the dock threads between them; on
	 * a desktop it drops below the zoom buttons, which leaves the left column and
	 * the whole bottom edge alone.
	 */
	.dock {
		position: fixed;
		z-index: 21;
		...
		gap: 0.4rem;
		inset: max(0.8rem, env(safe-area-inset-top)) 3.9rem auto 4.2rem;
		margin-inline: auto;
		width: min(100%, 22rem);
	}
	...
	.panel {
		position: fixed;
		z-index: 20;
		inset: calc(max(0.8rem, env(safe-area-inset-top)) + var(--spacing-touch) + 0.4rem) 0.75rem auto
			4.2rem;
		max-height: 62svh;
	...
	@media (min-width: 48rem) {
		...
		.panel {
			inset: calc(6.9rem + var(--spacing-touch) + 0.4rem) 0.9rem auto auto;
			width: 21rem;
